...
def _resolve_path(value: str, default_name: str) -> str:
    """Resolve a configured path against the project root.

    Env values may be absolute or relative. Relative values are
    interpreted relative to BASE_DIR (project root) so the project
    works from any CWD or checkout location.
    """
    if not value:
        return os.path.normpath(os.path.join(BASE_DIR, default_name))
...
DB_PATH = _resolve_path(os.environ.get("DB_PATH", ""), "tarefas.db")

# CAPTCHA Solver Configuration
CAPTCHA_SOLVER_PYTHON = os.environ.get("CAPTCHA_SOLVER_PYTHON", "") or sys.executable or "python3"
CAPTCHA_PREDICT_SCRIPT = _resolve_path(
    os.environ.get("CAPTCHA_PREDICT_SCRIPT", ""),
    os.path.join("captcha", "predict_captcha.py"),
)
CAPTCHA_MODEL_WEIGHTS = _resolve_path(
    os.environ.get("CAPTCHA_MODEL_WEIGHTS", ""),
    os.path.join("captcha", "best.pt"),
)

# Servidor HTTP (100% via env, sem host/porta fixos no código)
APP_HOST = os.environ.get("HOST", os.environ.get("APP_HOST", "0.0.0.0"))
