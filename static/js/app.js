...
function stopAllPolling() {
  if (state.matificJobPollInterval) {
    clearInterval(state.matificJobPollInterval);
    state.matificJobPollInterval = null;
  }
  if (state.matificBatchPollInterval) {
...
// Clears every per-user state so logging in as another student in the same
// tab never shows the previous account's tasks, episodes, books or job logs.
function resetClientState() {
  ...
  state.isEssay = false;
  state.currentMatificEp = null;
  state.currentMatificJobId = null;
  state.currentMatificBatchId = null;
  state.leiaspBooks = [];
  state.currentLeiaSPJobId = null;
  state.activeLeiaSPJob = null;
  ...
  if (el.inputSearchTasks) el.inputSearchTasks.value = '';
  if (el.toastContainer) el.toastContainer.innerHTML = '';
  if (el.tasksGrid) el.tasksGrid.innerHTML = '';
  if (el.essaysGrid) el.essaysGrid.innerHTML = '';
  if (el.matificAssignedGrid) el.matificAssignedGrid.innerHTML = '';
  if (el.matificAdventureGrid) el.matificAdventureGrid.innerHTML = '';
  if (el.leiaspGrid) el.leiaspGrid.innerHTML = '';
  if (el.batchTerminalLogs) el.batchTerminalLogs.innerHTML = '';
  if (el.matificSimLogs) el.matificSimLogs.innerHTML = '';
...
async function pollMatificBatch() {
  ...
  state.matificBatchPollInterval = setInterval(async () => {
    ...
    try {
      // Per-user batch status (there is no per-batch endpoint).
      const res = await fetch('/api/matific/batch/status');
      if (!res.ok) {
        if (res.status === 401 || res.status === 403 || res.status === 404) {
          clearInterval(state.matificBatchPollInterval);
          state.matificBatchPollInterval = null;
      ...
      const currentEp = (b.current_episode_slug || b.current_episode_title)
        ? { slug: b.current_episode_slug, title: b.current_episode_title }
        : null;
      ...
      if (currentEp) {
        el.batchCurrentTitle.textContent = `Executando: ${currentEp.title || currentEp.slug}`;
      } else {
...
function initModalMobileBehavior() {
  const modalPairs = [
    [el.modalTask, closeModal],
    [el.modalMatificSim, closeMatificModal],
    [el.modalMatificStats, () => { if (el.modalMatificStats) el.modalMatificStats.classList.add('hidden'); }],
    [el.modalLeiaSPReader, closeLeiaSPModal],
  ].filter(([overlay]) => overlay);

  modalPairs.forEach(([overlay, closeFn]) => {
  ...
  const syncBodyScrollLock = () => {
    const anyOpen = modalPairs.some(([overlay]) => !overlay.classList.contains('hidden'));
    document.body.classList.toggle('modal-open', anyOpen);
  };

  const observer = new MutationObserver(syncBodyScrollLock);
  modalPairs.forEach(([overlay]) => {
